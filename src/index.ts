...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    const corsHeaders = {
      ...
      "Access-Control-Allow-Headers": "Content-Type, Authorization, X-Requested-With, X-Mcp-Version, X-Mcp-Client-Info, X-Mcp-Post-Endpoint",
    ...
    if (url.pathname === "/" || url.pathname === "") {
      return new Response(
        JSON.stringify({
          ...
          // Debug info
          env_check: {
            has_api_token: !!env.CLICKUP_API_TOKEN,
            has_do_binding: !!env.MCP_OBJECT,
          }
        }, null, 2),
        {
          headers: {
    ...
    if (
      ...
      try {
        ...
        console.error("Durable Object interaction failed:", error);
